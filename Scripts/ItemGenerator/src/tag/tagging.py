...
def get_fluid_display_name(fluid_id):
    """Return a readable display name for a fluid identifier."""
...
def generate_fluid_tags(fluid_id, source_tags=None):
    """Generate Fluid.* tags from a vanilla/default fluid identifier."""
    normalized = _normalize_fluid_key(fluid_id)
    if not normalized:
...
def generate_fluid_container_tags(item_id, props):
    """Force a FluidContainer item through the container taxonomy."""
