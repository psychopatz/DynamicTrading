...
function DT_Reputation.ModifyFactionBias(factionID, amount, reason)
    ...
    if delta ~= 0 then
        ...
        Internal.ShowHalo("Faction Rep " .. prefix .. tostring(amount), delta > 0)
    end
...
function DT_Reputation.ModifyPersonalRep(traderUUID, factionID, amount, reason)
    ...
    if delta > 0 then
        Internal.ShowTraderHalo(traderUUID, "Rep +" .. tostring(amount), true, true)
    elseif delta < 0 then
        Internal.ShowHalo("Rep " .. tostring(amount), false)
    end
    if DT_Reputation.AUTO_DEBUG then
