...
local function syncNotice(player, message, severity)
    sendResponse(player, Config.COMMAND_MODULE, "LabourNotice", {
...
Network.Handlers.GiveWorkerMoney = function(player, args)
    ...
    if not worker then
        syncNotice(player, "That worker could not be found.", "error")
        return
    end

    if amount <= 0 then
        syncNotice(player, "Enter a valid amount of money to give.", "error")
        return
    end

    if not removePlayerMoney(player, amount) then
        syncNotice(player, "You do not have enough money for that transfer.", "error")
        return
    end

    Registry.AddMoney(worker, amount)
    Registry.Save()
    syncNotice(player, "Gave $" .. tostring(amount) .. " to " .. tostring(worker.name or worker.workerID) .. ".", "success")
    syncWorkerDetail(player, worker.workerID)
    syncWorkerList(player)
end
