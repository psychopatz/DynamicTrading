...
function Simulation.UpdateDaily()
    ...
    for id, faction in pairs(data) do
            ...
            if soul then
                ...
                if archData and archData.allocations then
                    for tag, score in pairs(archData.allocations) do
                        local resourceType = DynamicTrading.FactionConfig.ResourceMap[tag]
                        if resourceType then
                             -- Score * Multiplier (e.g., 6 * 2.0 = 12 units)
                            production[resourceType] = production[resourceType] + (score * DynamicTrading.FactionConfig.Sim.ProductionMultiplier)
                        end
                    end
                end
            end
        ...
        -- 1. CONSUMPTION
        local consumes = DynamicTrading.FactionConfig.Sim.BaseConsumption
        if consumes then
            ...
            if faction.stockpile.food < 0 then
                ...
                if faction.starvationDays >= deathThreshold then
                    -- Kill people
                    local deaths = math.ceil(faction.memberCount * DynamicTrading.FactionConfig.Sim.DeathRate)
            ...
            if faction.memberCount <= 0 then
                ...
                if surplusFood and ZombRand(100) < growthChance then
                    ...
                    if #archetypes > 0 and DynamicTrading_Engine.ConsumeRecruit() then
                        ...
                        faction.stockpile.food = faction.stockpile.food - DynamicTrading.FactionConfig.Sim.RecruitCost.food
