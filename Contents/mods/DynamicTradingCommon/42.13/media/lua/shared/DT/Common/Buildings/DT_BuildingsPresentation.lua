...
function Buildings.BuildOwnerSnapshot(ownerUsername)
    ...
    for buildingType, definition in pairs(Config.Definitions or {}) do
        local instances = {}
        local currentCount = 0
        for _, instance in ipairs(ownerData.buildings or {}) do
            if tostring(instance.buildingType or "") == tostring(buildingType) then
        ...
        local levels = {}
        for level, levelDefinition in pairs(definition.levels or {}) do
            local recipe = {}
        ...
        buildings[#buildings + 1] = {
            ...
            maxLevel = math.max(0, math.floor(tonumber(definition.maxLevel) or 0)),
            currentCount = currentCount,
            instances = instances,
            levels = levels,
            newBuildPreview = Buildings.BuildProjectPreview and Buildings.BuildProjectPreview(owner, buildingType, "build") or nil,
            upgradePreview = Buildings.BuildProjectPreview and Buildings.BuildProjectPreview(owner, buildingType, "upgrade") or nil
        }
    end
