...
DynamicTrading.Text = DynamicTrading.Text or {}

local Text = DynamicTrading.Text
...
local function getLanguageCode()
    if DynamicTrading.FlavorText and DynamicTrading.FlavorText.GetLanguage then
        return DynamicTrading.FlavorText.GetLanguage()
    end
...
local function getTableName(domain, lang)
    return "DT_Text_" .. tostring(domain) .. "_" .. tostring(lang)
end
...
function Text.LogMissing(key)
    ...
    local enabled = sandbox and sandbox.NPCDebug == true
    if not enabled and not (DynamicTrading.Config and DynamicTrading.Config.DebugText == true) then
        return false
    end

    Text._missingLogged[normalized] = true
    if DynamicTrading and DynamicTrading.Log then
        DynamicTrading.Log("DTCommons", "Text", "Missing", normalized)
    else
        print("[DynamicTrading.Text] Missing translation key: " .. normalized)
    end
    return true
end
