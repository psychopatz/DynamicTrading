...
function ISRadioWindow:readFromObject(_player, _deviceObject)
    -- If device changes, close our radar list because it may have stale data/range.
...
function ISRadioWindow:prerender()
    ...
    if self.btnTraderScan and self.btnTraderList then
        ...
        -- Only check cooldown when the radio is actually operational.
        -- This prevents OFF/no-power radios from displaying "Wait (1m)".
        ...
        -- Keep engine-enabled for white text and custom visuals.
        -- Actual OFF-radio click blocking is handled in the button callback.
        ...
        if not isEffectivelyDisabled then
            ...
            if isCooldownBlocked then
                ...
                -- Radio is OFF/no power, or the 3-second click grace is active.
                -- Show Scan instead of a fake cooldown.
                self.btnTraderScan:setTitle("Scan")
            end
